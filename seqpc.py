...
def csv_to_df(file_name):
    df = pd.read_csv(file_name, skiprows=5, header=None, usecols=range(2, 326))
    data = df.values
    pc_data = data.reshape(-1, 108, 3)
    pc_data_list = pc_data.tolist()

    labels = np.genfromtxt("labels.csv", delimiter=",", dtype=str)
    labels = labels.tolist()
    labels = [label[1:-1] for label in labels]
    
    df_pc = pd.DataFrame(pc_data_list, columns=labels)
    return df_pc
